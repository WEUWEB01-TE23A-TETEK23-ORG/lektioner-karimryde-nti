...
Här är koden för de tre kaotiska funktionerna. För att dessa ska fungera behöver du lägga till några rader i din HTML-fil och lite CSS för animationerna.

### Övning 3: Den blyga knappen

Den här knappen använder händelsen `onmouseover` för att hoppa undan så fort användaren försöker klicka på den.

**HTML (lägg till i knapp-panelen):**
```html
<button id="blyg-knapp" onmouseover="flyttaKnapp()">Subtrahera</button>
```
...
function flyttaKnapp() {
    ...
    // Slumpa fram ett värde mellan 0 och 200 pixlar
    let slump = Math.floor(Math.random() * 200);
    knapp.style.marginLeft = slump + "px";
    knapp.style.marginTop = slump + "px";
}
```

### Övning 4: Dramatisk exit

Den här funktionen raderar allt innehåll på webbsidan och lämnar användaren i mörker.

**HTML (lägg till längst ner i containern):**
```html
<button onclick="sjalvforstorelse()" style="background-color: darkred;">Självförstörelse</button>
```

**JavaScript:**
```javascript
function sjalvforstorelse() {
    if (confirm("Är du säker? Detta raderar allt!")) {
        document.body.innerHTML = "<h1 style='color: red; text-align: center; margin-top: 20%;'>SYSTEMFEL: Sidan har raderats.</h1>";
...
**CSS (lägg till i style.css):**
```css
/* Animation för att få containern att skaka */
@keyframes skaka {
...
**JavaScript (exempel med multiplikation):**
```javascript
function multiplicera() {
    ...
    if (produkt > 1000) {
        resultatText.textContent = "Hjälp! Talet är för stort!";
        container.classList.add("skakis"); // Lägger till CSS-klassen som skakar
    } else {
        resultatText.textContent = "Produkten är: " + produkt;
        container.classList.remove("skakis"); // Tar bort skakandet om man räknar ut något litet
    }
}
```
